#!/usr/bin/env bash
# Initialize terminal with Conda py311 Python runtime.
#
# Bash port of conda-py311-init-env.ps1 (PowerShell) — for when Claude Code
...
#
# This script:
#   1. Activates the conda `py311` environment (Python runtime only)
...
#   source ./scripts/bash/conda-py311-init-env.sh
#
...
#       "bash (Conda + uv)": {
#           "path": "bash",
#           "args": ["-c", "source '/home/<you>/repo/cr-misc-function/cr-misc-function/scripts/bash/conda-py311-init-env.sh'; cd './<project>/'; exec bash"]
#       }
#   },
...
echo ""
if [ -t 1 ]; then
    printf '\033[32m[OK] Conda py311 activated\033[0m\n'
else
    echo "[OK] Conda py311 activated"
fi
echo ""
python --version
